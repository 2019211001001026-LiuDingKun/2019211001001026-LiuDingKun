...
<ul>
    <c:forEach var="p" begin="1" end="10" step="2">
        <li>${p}</li>
    </c:forEach>
</ul>
...
personList.add(new Person("Liudingkun",new Dog("Sfdbeh")));
...
<ul>
    <c:forEach items="${AllPerson}" var="p2" >
        <li>person name=${p2.name}+dog name=${p2.dog.name}</li>
    </c:forEach>
</ul>
<%
...
<ul>
    <c:forTokens items="<%=str%>" var="p2" delims=":&&-">
                <li>${p2}</li>
    </c:forTokens>
</ul>
</body>
